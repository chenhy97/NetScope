...
/* CONSTANTS */
#define SKETCH_BUCKET_LENGTH 1440
...
#define MAX_REGISTER(num) register<bit<TS_SKETCH_CELL_BIT_WIDTH>>(SKETCH_BUCKET_LENGTH) max_value_sketch##num
#define MIN_REGISTER(num) register<bit<TS_SKETCH_CELL_BIT_WIDTH>>(SKETCH_BUCKET_LENGTH) min_value_sketch##num
...
// || meta.max_gap_value_sketch##num == meta.lat_ts_value_sketch##num
            ...
            // if (meta.latency < meta.quantile_value_sketch##num << 6){\
#define FILTER_PKTS_AT_BEGINING(num, algorithm)  hash(meta.prev_index_sketch##num, HashAlgorithm.algorithm, (bit<16>)0, {hdr.ipv4.srcAddr, \
...
#define PREV_LAT_TS_RECORD(num, algorithm)  hash(meta.prev_index_sketch##num, HashAlgorithm.algorithm, (bit<16>)0, {hdr.ipv4.srcAddr, \
    ...
    count_sketch##num.read(meta.count_value_sketch##num, meta.prev_index_sketch##num);\
    max_value_sketch##num.read(meta.max_value_sketch##num, meta.prev_index_sketch##num);\
    min_value_sketch##num.read(meta.min_value_sketch##num, meta.prev_index_sketch##num);\
    meta.tmp_max_gap_value##num = meta.max_gap_value_sketch##num;\
    if (meta.latency > meta.max_value_sketch##num){\
        if (meta.latency  > meta.max_value_sketch##num + meta.max_gap_value_sketch##num){\
                meta.tmp_max_gap_value##num = meta.latency - meta.max_value_sketch##num;\
        }\
        meta.max_value_sketch##num = meta.latency;\
    }\
    if (meta.latency < meta.min_value_sketch##num || meta.min_value_sketch##num == 0 ){\
        if (meta.min_value_sketch##num > meta.latency + meta.max_gap_value_sketch##num){\
             meta.tmp_max_gap_value##num = meta.min_value_sketch##num - meta.latency;\
        }\
        meta.min_value_sketch##num = meta.latency;\
    }\
    if (( meta.latency > meta.lat_ts_value_sketch##num &&\
        meta.latency - meta.lat_ts_value_sketch##num < meta.min_gap_value_sketch##num) \
    || meta.min_gap_value_sketch##num == 0){ \
        meta.min_gap_value_sketch##num = meta.latency - meta.lat_ts_value_sketch##num; \
    } \
    else if(( meta.latency < meta.lat_ts_value_sketch##num &&\
        meta.lat_ts_value_sketch##num - meta.latency < meta.min_gap_value_sketch##num) \
    || meta.min_gap_value_sketch##num == 0){ \
        meta.min_gap_value_sketch##num = meta.lat_ts_value_sketch##num - meta.latency; \
    } \
    max_gap_sketch##num.write(meta.prev_index_sketch##num, meta.tmp_max_gap_value##num); \
    min_gap_sketch##num.write(meta.prev_index_sketch##num, meta.min_gap_value_sketch##num); \
    meta.lat_ts_value_sketch##num = (meta.latency); \
    ...
    max_value_sketch##num.write(meta.prev_index_sketch##num,  meta.max_value_sketch##num);\
    min_value_sketch##num.write(meta.prev_index_sketch##num, meta.min_value_sketch##num);\
...
#define QUANTILE_UPDATE(num) \
    lat_quantile_sketch##num.read(meta.quantile_value_sketch##num, meta.prev_index_sketch##num);\
    if (meta.quantile_value_sketch##num == 0){\
        meta.quantile_value_sketch##num = meta.latency;\
        ...
        if (meta.latency < meta.quantile_value_sketch##num){ \ 
            meta.lambda_value##num = ((meta.max_gap_value_sketch##num >> 4) + (meta.min_gap_value_sketch##num >> 1) );\ 
            if (meta.c_minus_value_sketch##num + 1 > meta.count_value_sketch##num - meta.percentile_result){\
                if (meta.quantile_value_sketch##num > meta.lambda_value##num + (meta.lambda_value##num>>2)){\
                    meta.quantile_value_sketch##num = meta.quantile_value_sketch##num - meta.lambda_value##num;\
                    meta.c_plus_value_sketch##num = meta.c_plus_value_sketch##num + 1;\
                }\
                else{\
                    meta.quantile_value_sketch##num = meta.quantile_value_sketch##num;\
                    meta.max_value_sketch##num = meta.latency;\
                }\
            }\
            else{\
        ...
        else if(meta.latency > meta.quantile_value_sketch##num){\
            meta.lambda_value##num = ((meta.max_gap_value_sketch##num >> 3) + meta.min_gap_value_sketch##num );\ 
...
control MyEgress(inout headers hdr,
    ...
    action add_int_header(){
        ...
        hdr.int_hdr.prev_index_sketch0 = meta.prev_index_sketch0;
        hdr.int_hdr.prev_index_sketch1 = meta.prev_index_sketch1;
        hdr.int_hdr.prev_index_sketch2 = meta.prev_index_sketch2;

        hdr.int_hdr.latency = meta.latency;
        hdr.int_hdr.lambda0 = meta.lambda_value0;
        hdr.int_hdr.lambda1 = meta.lambda_value1;
        hdr.int_hdr.lambda2 = meta.lambda_value2;
        ...
        hdr.int_hdr.lat_ts_value_sketch0 = meta.lat_ts_value_sketch0;
        hdr.int_hdr.lat_ts_value_sketch1 = meta.lat_ts_value_sketch1;
        hdr.int_hdr.lat_ts_value_sketch2 = meta.lat_ts_value_sketch2;
        hdr.int_hdr.max_value_sketch0 = meta.max_value_sketch0;
        hdr.int_hdr.max_value_sketch1 = meta.max_value_sketch1;
        hdr.int_hdr.max_value_sketch2 = meta.max_value_sketch2;
        hdr.int_hdr.min_value_sketch0 = meta.min_value_sketch0;
        hdr.int_hdr.min_value_sketch1 = meta.min_value_sketch1;
        hdr.int_hdr.min_value_sketch2 = meta.min_value_sketch2;
        // hdr.int_hdr.temp_ingr_ts = meta.temp_ingr_ts;
        // hdr.int_hdr.temp_egr_ts = meta.temp_egr_ts;
    ...
    action percentile_calc(){
        meta.percentile_result = meta.count_value_sketch >> 1;
    }
    ...
    apply { 
        ...
        if (hdr.ipv4.isValid() && hdr.tcp.isValid()){
            ...
            // FILTER_PKTS_AT_BEGINING(0, crc16_custom);
            // FILTER_PKTS_AT_BEGINING(1, crc16_custom);
            // FILTER_PKTS_AT_BEGINING(2, crc16_custom);
            prev_sketch_calc();
            cmp_count_value_register();
            percentile_match.apply();
            // percentile_calc();
