"""This module includes all the features needed to query the ``config.json`` file
...
class InvalidHostIP(Exception):
    ...
    def __init__(self, ip):
        ...
        super(InvalidHostIP,
              self).__init__('InvalidHostIP: {}'.format(self.message))
...
class IntfDoesNotExist(Exception):
    ...
    def __init__(self, arg1, arg2, mode=0):
        if mode == 0:
            self.message = 'interface <{}> does not exist on node <{}>.'.format(
                arg1, arg2)
        elif mode == 1:
            self.message = 'no interface facing node <{1}> exists on node <{0}>.'.format(
                arg1, arg2)
...
class NetworkGraph(nx.Graph):
    ...
    def _populate_dicts(self):
        ...
        for node in self.nodes:
            ...
            for neighbor in self.neighbors(node):
                self.edge_to_intf[node][neighbor] = self._edge_to_intf(
                    node, neighbor)
                self.edge_to_intf[neighbor][node] = self._edge_to_intf(
                    neighbor, node)
                self.node_to_intf[node][self.edge_to_intf[node][neighbor][
                    'intfName']] = self.edge_to_intf[node][neighbor]
                self.node_to_intf[neighbor][self.edge_to_intf[neighbor][node][
                    'intfName']] = self.edge_to_intf[neighbor][node]
        ...
        if len(fields) > 0:
            ...
            for node_a in self.edge_to_intf:
                ...
                for node_b in self.edge_to_intf[node_a]:
                    ...
                    for field in fields:
                        reduced_intfs[node_a][node_b].append(
                            self.edge_to_intf[node_a][node_b].get(field))
                    reduced_intfs[node_a][node_b] = tuple(
                        reduced_intfs[node_a][node_b])
                    if len(fields) == 1:
                        reduced_intfs[node_a][node_b] = reduced_intfs[node_a][
                            node_b][0]
        ...
        if len(fields) > 0:
            ...
            for node in self.node_to_intf:
                ...
                for intf in self.node_to_intf[node]:
                    ...
                    for field in fields:
                        reduced_intfs[node][intf].append(
                            self.node_to_intf[node][intf].get(field))
                    reduced_intfs[node][intf] = tuple(
                        reduced_intfs[node][intf])
                    if len(fields) == 1:
                        reduced_intfs[node][intf] = reduced_intfs[node][intf][
                            0]
            return reduced_intfs
        else:
    ...
    def get_switches(self, fields=[]):
        ...
        if len(fields) > 0:
            ...
            for switch in switches:
                ...
                for field in fields:
                    reduced_switches[switch].append(
                        switches[switch].get(field))
    ...
    def get_p4switches(self, fields=[]):
        ...
        Example:
            Let us consider the following example to clarify::
                ...
                {'s1': False, ...}
        """
        p4switches_subgraph = nx.subgraph_view(self,
                                               filter_node=self.isP4Switch)
        ...
        if len(fields) > 0:
            ...
            for p4switch in p4switches:
                ...
                for field in fields:
                    reduced_p4switches[p4switch].append(
                        p4switches[p4switch].get(field))
                reduced_p4switches[p4switch] = tuple(
                    reduced_p4switches[p4switch])
                if len(fields) == 1:
                    reduced_p4switches[p4switch] = reduced_p4switches[
                        p4switch][0]
    ...
    def get_p4rtswitches(self, fields=[]):
        ...
        Example:
            Let us consider the following example to clarify::
                ...
                {'s1': False, ...}
        """
        p4rtswitches_subgraph = nx.subgraph_view(
            self, filter_node=self.isP4RuntimeSwitch)
        ...
        if len(fields) > 0:
            ...
            for p4rtswitch in p4rtswitches:
                ...
                for field in fields:
                    reduced_p4rtswitches[p4rtswitch].append(
                        p4rtswitches[p4rtswitch].get(field))
                reduced_p4rtswitches[p4rtswitch] = tuple(
                    reduced_p4rtswitches[p4rtswitch])
                if len(fields) == 1:
                    reduced_p4rtswitches[p4rtswitch] = reduced_p4rtswitches[
                        p4rtswitch][0]
    ...
    def get_cpu_port_intf(self, name, quiet=False):
        ...
        """
        if self.isP4Switch(name) and self.get_nodes()[name].get(
                'cpu_port', False):
    ...
    def get_thrift_ip(self, name):
        ...
        if self.isP4Switch(name):
            print(
                'This method is not yet fully implemented, all switches listen on 0.0.0.0.'
            )
            return '0.0.0.0'
    ...
    def get_host_gateway_name(self, name):
        ...
        if self.isHost(name):
            return self._node_interface(
                name, self.get_host_first_interface(name))['node_neigh']
        else:
            raise TypeError('{} is not a host.'.format(name))
    ...
    def total_number_of_paths(self):
        ...
        for host in self.get_hosts():
            for host_pair in self.get_hosts():
                ...
                npaths = sum(1 for _ in nx.all_shortest_paths(
                    self, host, host_pair, 'weight'))
                total_paths += npaths
        return total_paths
    ...
    def keep_only_p4switches_and_hosts(self):
        ...
        return self.subgraph(
            list(self.get_p4switches().keys()) + list(self.get_hosts().keys()))
